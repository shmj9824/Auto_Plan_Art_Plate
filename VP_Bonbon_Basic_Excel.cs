...
{
    internal class VP_Bonbon_Basic_Excel : Call_VP_Excel
    {
        //public string _test_str;
        public List<Bonbon_Basic_vpdata> bonbon_Vps = new List<Bonbon_Basic_vpdata>();
        ...
        Dictionary<string, (string, List<string>)> dic_Ingredient = new Dictionary<string, (string, List<string>)>();
        public VP_Bonbon_Basic_Excel(VP_Mode mode_str) : base(mode_str)
        { }
        public VP_Bonbon_Basic_Excel(VP_Mode mode_str, Dictionary<string, (string, List<string>)> include1) : base(mode_str)
        {
        ...
        public override void VP_check()
        {
            ...
            for (int u = 1; u <= wb_excel.Worksheets.Count; u++)
            {
                ...
                while (Convert.ToString(ws_excel.Cells[product_number_row, 1].value) != null)
                {
                    ...
                    spandex_value *= 100;
                    string ingredient1 = "vải " + Convert.ToString(ws_excel.Cells[product_number_row, 6].value) + " "
                        + nylon_value.ToString() + "% "
                        + Convert.ToString(ws_excel.Cells[product_number_row, 8].value) + " "
                        + spandex_value.ToString() + "%";
                    product.Add(ingredient1);
                    //ingredient2
                    string ingredient2 = "";
                    product.Add(ingredient2);
                    //ingredient3
                    string ingredient3 = "";
                    product.Add(ingredient3);
                ...
                while (Convert.ToString(ws_excel.Cells[data_row, check_col].value) != null)
                {
                    ...
                    int product_art_no = Convert.ToInt32(q_name[5]);
                    ...
                    art_no = Convert.ToString(ws_excel.Cells[Convert.ToInt32(q_name[6]), get_col_num].value);
                    bonbon_Vps.Add(new Bonbon_Basic_vpdata(product
                        , color, size, barcode
                        , q_name[0], q_name[1], q_name[2], q_name[3], q_name[4], art_no));
                    data_row++;
                }
            }
        }
            ...
            while (ws_excel.Cells[row_end, 4].text != "")
            {
                try
                {
                    ...
                    */
                    ...
                    List<string> ingredient = dic_Ingredient[art_no].Item2;

                    bonbon_Vps.Add(new Bonbon_Basic_vpdata(product_num, color, size, barcode, price, item, ingredient[0], ingredient[1], ingredient[2], art_no));
    ...
    class Bonbon_Basic_Reference_file : Call_Excel
    {
        public List<Bonbon_Basic_vpdata> bonbon_Vps;
        public Dictionary<string, (string, List<string>)> art_no_Ingredient = new Dictionary<string, (string, List<string>)>();
        string mode;
        public Bonbon_Basic_Reference_file(List<Bonbon_Basic_vpdata> bonbon_Vps, string mode)
        {
            this.bonbon_Vps = bonbon_Vps;
            this.mode = mode;
        }
        public Bonbon_Basic_Reference_file(string mode)
        {
            this.mode = mode;
        }
        ...
        public override void Load_File()
        {
            if (mode == "write")
            {
                ...
                ws_excel = wb_excel.Worksheets[1];
                int row_dock = 2;
                foreach (Bonbon_Basic_vpdata _Vp_Data in bonbon_Vps)
                {
                    if (_Vp_Data.Art_no != temp_art_no || _Vp_Data.Product_num != temp_product)
                    {
                        ...
                        //三成份
                        ws_excel.Cells[row_dock, 5].value = _Vp_Data.Ingredient1;
                        ws_excel.Cells[row_dock, 6].value = _Vp_Data.Ingredient2;
                        ws_excel.Cells[row_dock, 7].value = _Vp_Data.Ingredient3;
                        row_dock++;
                    }
                }
            }
            ...
            {
                bonbon_Vps = new List<Bonbon_Basic_vpdata>();
                int row_end = 2;
                ws_excel = wb_excel.Worksheets[1];
                ...
                while (ws_excel.Cells[row_end, 1].text != "" || Convert.ToString(ws_excel.Cells[row_end, 1].text).Contains("-"))
                {
                    ...
                    {
                        Convert.ToString(ws_excel.Cells[row_end, 5].value),
                        Convert.ToString(ws_excel.Cells[row_end, 6].value),
                        Convert.ToString(ws_excel.Cells[row_end, 7].value)
                    };

                    art_no_Ingredient.Add(art_no, (item, list_Ingredient));
                    row_end++;
                }
            }
        }
    ...
    class Bonbon_Basic_vpdata
    {
        ...
        public Bonbon_Basic_vpdata(string Product, string color, string size, string barcode, string prize, string it, string ingredient1, string ingredient2, string ingredient3, string an)
        {
